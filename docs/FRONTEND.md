...
- **Species dashboard**: Score de patrimonialité (valeur de conservation) + driver badges displayed prominently near title. `computeSensitivity()` (dans `lib/sensitivity.ts`) agrège écologique (Liste rouge) + réglementaire (protections/directives/conventions) + territorial (ZNIEFF/PNA) ; l'EEE reste un badge informatif hors score. Émet une pastille par (cdTypeStatut × territoire) — "LRN EN", "LRR EN", "PN/PR/PD/POM", "DH/DO" nommées, "ZNIEFF (territoire)", "PNA/exPNA" — avec tooltips détaillées.
...
- **Habitats (deux sections distinctes)**: « Habitats (EUNIS) » = milieux de vie (lazy-cachés en ligne) ; « Habitats associés (HABREF) » = types d'habitats EUNIS dont l'espèce est caractéristique (bulk offline, table `habref_habitats`). Les deux coexistent, repliées par défaut.
