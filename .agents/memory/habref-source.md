...
- **Données** : le Publish ne migre QUE le schéma, pas les lignes. Chemin retenu pour charger `habref_habitats` en prod sans manipuler la DATABASE_URL prod : endpoint admin `POST /api/admin/habref/import` (auth ADMIN_TOKEN **avant** le parser 32 Mo route-scoped), qui remplace la table de façon atomique. On parse les CSV en dev (`scripts/src/import-habref.ts`), puis on exporte les lignes dev (`psql ... json_agg({cdRef,habitats})`) et on les POST vers l'app déployée. Data-write en prod passe TOUJOURS par l'app déployée (comme `cache/clear`), jamais depuis le sandbox (réplica read-only).
- **Why:** un webapp Replit à data lourde ne charge pas ses données via le Publish ; toute nouvelle source bulk exige une étape d'import prod séparée, et le seul chemin d'écriture prod depuis l'agent est un endpoint admin dans l'app déployée.
